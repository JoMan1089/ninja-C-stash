...
void string_toupper(char str[], size_t len)
{
	unsigned long i;
	for (i = 0; i < (len); i++) {
		str[i] = toupper(str[i]);
...
void string_tolower(char str[], size_t len)
{
	unsigned long i;
	for (i = 0; i < (len); i++) {
		str[i] = tolower(str[i]);
...
void string_toproper(char str[], size_t len)
{
	string_tolower(str, len);
	str[0] = toupper(str[0]);
}

int string_rm_all_space(char str[], size_t len)
{
	unsigned long i;
	int space_removed = 0;
	for (i = 0; i <= (len - 1); i++) {
		if ((isspace(str[i])) && (i !=0)) {
			string_backspace(str, len, i);
			space_removed += 1;
			i--;
			len--;
		}
...
void string_backspace(char str[], size_t len, unsigned long index)
{
	unsigned long i;
...
int strncmp_case(char str1[], char str2[], size_t n)
{
	...
	string_tolower(buff1, strlen(buff1));
	string_tolower(buff2, strlen(buff2));
	return strncmp(buff1, buff2, n);
}
bool string_isdigit(char str[], size_t len) {
	if (str[0] == '\0') {
		return false;
	}
	else {
...
bool string_isradix(char str[], size_t len)
{
	if (str[0] == '\0') {
		return false;
	}
	else {
		unsigned long i, radix_ctr;
		bool negative = false;
...
size_t string_len(char str[], size_t size)
{
	unsigned long i;
	for (i = 0; (i < size - 1); i++) {
		if (str[i] == '\0') {
